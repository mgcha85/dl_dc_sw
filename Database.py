...
class Database:
    ...
    def get_dairly_data(self, code, date, interval='month'):
        if interval == 'month':
            con = sqlite3.connect(os.path.join(self.root, self.fname + '_{}-{}.db'.format(date.year, date.month)),
                                  check_same_thread=False)
        elif interval == 'year':
            con = sqlite3.connect(os.path.join(self.root, self.fname + '_{}.db'.format(date.year)),
                                  check_same_thread=False)
        else:
            return
        df = pd.read_sql("SELECT * FROM {tn} WHERE Date BETWEEN '{date}' AND '{date}'".format(tn=code, date=date),
                               con, index_col='Date')
    ...
    #     if interval == 'month':
    #         dates = [dt.date() for dt in rrule(WEEKLY, dtstart=date1, until=date2)]
    #         dates.append(date2)
    #         return np.unique(np.array(['{:04d}-{:02d}'.format(x.year, x.month) for x in dates]))
    #     elif interval == 'year':
    #         dates = [dt.date() for dt in rrule(MONTHLY, dtstart=date1, until=date2)]
    #         dates.append(date2)
    #         return np.unique(np.array(['{:04d}'.format(x.year) for x in dates]))

    def get_range(self, date1, date2, interval='month'):
        date1 = date1.replace(day=1)

        if interval == 'month':
            date2 += relativedelta(months=1)
            date2 = date2.replace(day=1)
            date2 -= relativedelta(days=1)
            dates = [dt.date() for dt in rrule(MONTHLY, dtstart=date1, until=date2)]
            return np.array(['{:04d}-{:02d}'.format(x.year, x.month) for x in dates])
        elif interval == 'year':
            date2 += relativedelta(years=1)
            date2 = date2.replace(day=1)
            date2 -= relativedelta(days=1)
            dates = [dt.date() for dt in rrule(YEARLY, dtstart=date1, until=date2)]
            return np.array(['{:04d}'.format(x.year) for x in dates])

    def get_data_by_datetime(self, code, date1, date2, time1, time2, interval='month', range=None):
        if range is None:
            range = self.get_range(date1, date2, interval)
        dfs = []
        for date in range:
            con = sqlite3.connect(os.path.join(self.root, 'database', self.fname + '_{}.db'.format(date)),
                                  check_same_thread=False)
            if Database.checkTableExists(con, code) is False:
                continue

            df = pd.read_sql(
                "SELECT * FROM '{tn}' WHERE Date BETWEEN '{date1} {time1}' AND '{date2} {time2}'".format(tn=code,
                                                                                                         date1=date1,
                                                                                                         date2=date2,
                                                                                                         time1=time1,
                                                                                                         time2=time2),
                con, index_col='Date').astype(float)
            df.index = pd.to_datetime(df.index)
            dfs.append(df)
        if dfs:
    ...
    def get_data_by_date(self, code, date1, date2, interval='month', range=None):
        if range is None:
            range = self.get_range(date1, date2, interval)
        dfs = []
        for date in range:
            fpath = os.path.join(self.root, 'database', self.fname + '_{}.db'.format(date))
            if not os.path.exists(fpath):
                continue

            con = sqlite3.connect(fpath, check_same_thread=False)
            if self.checkTableExists(con, code) is False:
                continue

            df = pd.read_sql("SELECT * FROM '{tn}' WHERE Date BETWEEN '{date1}' AND '{date2}'"
                                   "".format(tn=code, date1=date1, date2=date2), con, index_col='Date')
            df.index = pd.to_datetime(df.index)
            dfs.append(df)
    ...
    def get_data(self, code, interval='month'):
        today = TimeManager.get_today()

        if interval == 'month':
            con = sqlite3.connect(
                os.path.join(self.root, 'database', self.fname + '_{}-{}.db'.format(today.year, today.month)),
                check_same_thread=False)
        elif interval == 'year':
            con = sqlite3.connect(os.path.join(self.root, 'database', self.fname + '_{}.db'.format(today.year)),
                                  check_same_thread=False)
        else:
            return
    ...
    def get_dateList(root):
        ...
        flist = [x for x in flist if 'intra_day_3min' in x]
    ...
    def get_sample(root):
        today = TimeManager.get_today()
        con = sqlite3.connect(
            os.path.join(root, 'database', 'intra_day_3min_{:04d}-{:02d}.db'.format(today.year, today.month)),
            check_same_thread=False)
        df = pd.read_sql("SELECT * FROM '038500'", con, index_col='Date')
        df.index = pd.to_datetime(df.index)
        return df
